...
export class Notice extends Base {
	...
	getStepper() {
		...
		return {
			getLabel(step) {
				const size = self.steps.length,
					current = self.steps.findIndex((el) => step === el) + 1;

				return `Step: ${current}/${size}`;
			},
			getNext(step) {
				const current = self.steps.findIndex((el) => step === el) + 1;
	...
	dismissPanel() {
		...
		if ('default' !== this.settings.template.choice && !BoldgridEditor.is_boldgrid_theme) {
			const val = 'template/page/' + this.settings.template.choice + '.php';
	...
	saveSettings() {
		const $inputs = BG.Panel.$element.find('input, select'),
		...
		$.ajax({
			...
			const inputName = `bgppb_post_type[${BoldgridEditor.post_type}]`,
